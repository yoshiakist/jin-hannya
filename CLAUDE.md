...
- 経文の外のページ（このサイトについて）は **`kind: page` のノード**として置き、L1 と同じ画面文法・同じ遷移で読ませる。木にも隣接にも繋がず（繋ぐと語の一覧に紛れる）、経路だけ `ancestryOf` が根に繋ぐ。入口は L0 左上の固定リンクで、L1 以降では出さない。原稿の小見出し `##` と外部リンク `[字面](URL)` は**このページの原稿だけ**が使う。
  `kind: page` の `range` は**持ち越す字の指定**（`about` の `深般若` ← `行深般若波羅蜜多時`）で、語の宣言ではない。紙面での所有は句のままなので hover も入口も読破も変わらず、動くのは遷移だけ（→ skill: `transition-fx`）。
