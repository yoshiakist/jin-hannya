...
前計算した bin は起動時にまとめて取ると数 MB になり、**取り終えるまで Canvas がマウントされない＝ロゴの手前に黒い画面が入る**。落とし所は 2 つ。

- **最初の一画面に要るものだけを待つ。** `loadGlyphs()` が取るのは `mesh` / `sdf` / `order` だけ。粒子は最初の遷移まで要らないので、レンダラが立ってティアが訂正されたあとにロゴの裏で取る（`loadParticles()` / `Stage.tsx` の `ParticleLoader`）。届く前に潜っても演出は崩れない（`glyphParticles` が `null` を返し、その回だけ粒子が出ない）。**粒子が取れなくても紙面へは落とさない**（`stageFailedAtom` は最初の一画面が成立しないときのためのもの）。
- **ティアが使う量だけを送る。** 粒子は `particles-lo`（Tier 2 の 1 字 400 点）と `particles-hi`（Tier 1 の全量）の 2 本に分けて焼き、ティアに合う側だけを取る。lo は hi の**先頭を切っただけ**で、サンプルが面積重みで一様なので分布は保たれる。

座標列は f32・u32 のままだと brotli がほとんど効かない（粒子 4.1MB → 3.8MB）ので、**表現そのものを詰める**。粒子は u16 に量子化（刻みは字面の一辺の 1/65535）、メッシュのインデックスも u16（字ごとに 0 から振り直しているので収まる。ビルド時に確かめている）。

結果、起動を止めるのは 5.0MB、Tier 2 が生涯で取るのは 5.2MB（もとは全ティア 10.8MB）。
