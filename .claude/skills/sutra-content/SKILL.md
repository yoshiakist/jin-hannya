...
- **`range` は「持ち越す字」の指定であって、語の宣言ではない。** `about` の `range: [16, 19]` は、大書の `深般若` が全文のどこから来た字か（`行深般若波羅蜜多時`）を指す。
  L0 からリンクで出入りするとき、その 3 字が紙面から大書へ抜けていく／戻っていく（→ `transition-fx`）。
  **紙面での所有は句のまま**で、hover も入口も読破も変わらない（`SUTRA_INDEX_TO_NODE` には入れない）。
  `深般若` は経文の語のまとまりではない（`深` は `行` に掛かる）ので、紙面で一続きの語として見せると読みを一つ間違って教えることになる。動きは文法の主張ではないので、そこにだけ使う。
