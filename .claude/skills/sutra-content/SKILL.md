...
「このサイトについて」のような、経文のグラフに属さないページ。**L1 と同じ画面文法・同じ遷移**で読ませたいので、専用の画面を作らずノードとして置く。

```yaml
id: about
kind: page
label: 深般若              # 大書。字は他と同じくグリフ在庫が要る
reading: じんはんにゃ
summary: …                # 大書の左。他のノードと同じ役割
layout: none              # 子を持たないので none 固定
```

- **木にも隣接にも繋がない**（`parent` / `anchor` / `children` / `related` を持たない）。繋ぐと語の一覧や関連語句に紛れ込み、経文の読み筋が濁る。スキーマが弾く。
- 経文に現れないので `range` も持たない。
- 経路（URL・現在位置インジケータ・戻り先）は `ancestryOf` が根に繋ぐので `/about/` になり、深さは L1 と同じに見える。到達不能の検査ではそれ自身を起点として扱う（入口は L0 左上の固定リンク → `navigation-fsm`）。
- 原稿は `docs/<id>.md`。ここだけは**小見出し（`##`）と外部リンク（`[字面](URL)`）を使ってよい**（→ `doc-writing`）。
...
- `kind: page` が木・隣接・`range` を持たないこと（`layout: none` であること）
